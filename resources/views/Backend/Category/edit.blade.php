                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('projectCategories.update',$projectCategory->id) }}" method="post" >
                            @method('PUT') @csrf
                            <input type="text" name="name" value="{{ old('name',$projectCategory->name) }}" class="form-control">
                        ...
                        <table class="table table-striped  ">
                            ...
                            @foreach($categories as $g)
                                <tr>
                                    ...
                                    <td class="d-flex ">
                                        <a href="{{ route('projectCategories.edit',$g->id) }}" class="pe-7s-pen text-decoration-none btn btn-sm  fsize-1 "></a>
                                        <form id="deleteGenre{{ $g->id }}" action="{{ route('projectCategories.destroy',$g->id) }}" method="post" class="mx-2  ">
                                            @csrf @method('DELETE')
                                        </form>
